...
async function ajouterMenuMystere() {
    // On cible le bouton correctement pour l'animation
    const btn = document.querySelector('button[onclick="ajouterMenuMystere()"]');
    if(btn) {
    ...
    try {
        ...
        const reponse = await fetch('api_get_plats.php');
        const data = await reponse.json();
        const plats = data.plats; // On ne prend que les plats, pas les menus prédéfinis

        // On trie les plats par catégories
        const pizzas = plats.filter(p => p.categorie === 'Pizza');
        const boissons = plats.filter(p => p.categorie === 'Boisson');
        const desserts = plats.filter(p => p.categorie === 'Dessert');

        //On tire au sort un article dans chaque catégorie
        const pizzaMystere = pizzas[Math.floor(Math.random() * pizzas.length)];
        const boissonMystere = boissons[Math.floor(Math.random() * boissons.length)];
        const dessertMystere = desserts[Math.floor(Math.random() * desserts.length)];

        // On crée le Menu de toutes pièces !
        const nomCombine = `✨ Menu Mystère (${pizzaMystere.nom}, ${boissonMystere.nom}, ${dessertMystere.nom})`;
        const prixMenu = 14.99; // Prix fixe du menu mystère
        const idMenu = "MYSTERE-" + Math.floor(Math.random() * 10000);

        // On l'envoie au serveur 
        const ajoutReponse = await fetch('api_add_cart.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: idMenu, nom: nomCombine, prix: prixMenu })
        });

        const dataRetour = await ajoutReponse.json();

        if (dataRetour.success) {
            // Affichage de la surprise au client !
            alert(`🎉 SURPRISE ! Vous avez obtenu :\n\n🍕 ${pizzaMystere.nom}\n🥤 ${boissonMystere.nom}\n🍰 ${dessertMystere.nom}\n\nLe tout pour ${prixMenu}€ !`);
            
            // On lance la petite notification visuelle verte
            creerToastNotification(`🛒 Menu Mystère ajouté !`);
        } else {
            alert("Erreur du serveur : " + dataRetour.message);
        }

    } catch (erreur) {
        console.error("Crash du Menu Mystère :", erreur);
        alert("Oups, la roulette a déraillé !");
    } finally {
        // On remet le bouton à son état normal
        if(btn) {
            btn.innerText = "✨ Menu Mystère";
            btn.disabled = false;
        }
    }
}
